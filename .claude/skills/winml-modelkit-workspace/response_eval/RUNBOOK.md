...
This RUNBOOK is **agent-facing** — when a user says "iterate response", "add a response case", or "let's push <UTC-datetime>", the agent driving the conversation follows these steps. Each run is a snapshot at `response_eval/runs/<UTC-datetime>/`; "iteration" in prose refers to the activity of refining the skill, not the directory name.
...
Each case is graded twice per run: **with_skill** (skill loaded) and **without_skill** (baseline). The delta tells us the skill is actually doing work.
...
```
response_eval/runs/<UTC-datetime>/benchmark.md           — latest run's pass rate
response_eval/runs/<UTC-datetime>/eval-*/grading.json    — per-case results
...
Use the current UTC datetime as the new run's name (format `YYYYMMDD-HHMMSS`). The previous run's directory becomes the source for layout + baselines.

```bash
cd response_eval/runs
...
```python
# runs/<new-UTC-datetime>/grade.py
...
```bash
python runs/<new-UTC-datetime>/grade.py
```
...
4. Create the per-run directory for the next run:
   ```
...
5. Spawn BOTH with_skill AND baseline subagents for the new case (baseline can't be reused from previous run since this case didn't exist).
