...
Usage in pytest fixtures:
    from tests.optim.assets.generate_fusion_patterns import (
...
def make_compatible_model(
    graph: onnx.GraphProto, opset_version: int = 14
) -> onnx.ModelProto:
...
def create_groupnorm_model(
    ...
    gn_shape = np.array(
        [batch_size, num_groups, channels_per_group, height, width], dtype=np.int64
    )
