...
def slice_elimination_builder(
    ...
    initializers.append(
        numpy_helper.from_array(
            np.array([INT64_MAX, INT64_MAX], dtype=np.int64), f"{prefix}ends"
        )
    )
    # Axes in any order, as long as all dims are covered
    initializers.append(
        numpy_helper.from_array(np.array([0, 1], dtype=np.int64), f"{prefix}axes")
    )
    ...
    return [
        ...
        helper.make_node(
            "Abs", [f"{prefix}slice_out"], [f"{prefix}abs_out"], name=f"{prefix}abs"
        ),
        helper.make_node(
            "Relu", [f"{prefix}abs_out"], [output_name], name=f"{prefix}relu"
        ),
    ]
...
def unsqueeze_elimination_builder(
    ...
    initializers.append(
        numpy_helper.from_array(const_data, f"{prefix}const_tensor")
    )
    ...
    return [
        ...
        helper.make_node(
            "Relu", [f"{prefix}add_out"], [output_name], name=f"{prefix}relu"
        ),
    ]
...
def expand_elimination_builder(
    ...
    return [
        ...
        helper.make_node(
            "Relu", [f"{prefix}expand_out"], [output_name], name=f"{prefix}relu"
        ),
    ]
...
def concat_slice_elimination_builder(
    ...
    initializers.append(
        numpy_helper.from_array(np.array([0], dtype=np.int64), f"{prefix}starts")
    )
    initializers.append(
        numpy_helper.from_array(np.array([20], dtype=np.int64), f"{prefix}ends")
    )
    initializers.append(
        numpy_helper.from_array(np.array([0], dtype=np.int64), f"{prefix}axes")
    )
    initializers.append(
        numpy_helper.from_array(np.array([1], dtype=np.int64), f"{prefix}steps")
    )

    # Pad to restore shape for template compatibility: [20] -> [64]
    initializers.append(
        numpy_helper.from_array(np.array([0, 44], dtype=np.int64), f"{prefix}pads")
    )
    initializers.append(
