...
class TestXrtSmiClient:
    ...
    def test_get_command_submissions_sums_across_contexts(self):
        ...
        contexts = [
            HwContext(
                pid=100, context_id=0, status="Active",
                command_submissions=50, command_completions=50,
                gops="N/A", fps="N/A", latency="N/A",
                priority="Low", errors=0,
            ),
            HwContext(
                pid=100, context_id=1, status="Idle",
                command_submissions=30, command_completions=30,
                gops="N/A", fps="N/A", latency="N/A",
                priority="Low", errors=0,
            ),
        ]
...
class TestLiveMonitorDisplay:
    ...
    def test_render_status_warmup_phase(self):
        ...
        display = LiveMonitorDisplay(
            total_iterations=110, warmup=10, model_id="test", device="npu"
        )
        status = display._render_status(
            iteration=5, latency_ms=1.0, util_samples=[50.0],
            memory_local_mb=10.0, memory_shared_mb=20.0,
            cpu_pct=5.0, ram_mb=8000.0,
        )
    ...
    def test_render_status_benchmark_phase(self):
        ...
        status = display._render_status(
            iteration=50, latency_ms=2.0, util_samples=[80.0, 90.0],
            memory_local_mb=31.0, memory_shared_mb=43.0,
            cpu_pct=15.0, ram_mb=40000.0,
        )
    ...
    def test_render_status_zero_latency_no_crash(self):
        ...
        display = LiveMonitorDisplay(
            total_iterations=10, warmup=0, model_id="test", device="cpu"
        )
        # latency_ms=0 should not cause division by zero
        status = display._render_status(
            iteration=1, latency_ms=0.0, util_samples=[],
        )
        assert "Throughput" in status
    ...
    def test_render_status_empty_samples(self):
        ...
        status = display._render_status(
            iteration=1, latency_ms=1.0, util_samples=[],
        )
    ...
    def test_update_noop_when_live_is_none(self):
        ...
        display.update(
            iteration=1, latency_ms=1.0, util_samples=[50.0],
        )
    ...
    def test_print_final_snapshot_is_noop(self):
        ...
        display.print_final_snapshot(
            util_samples=[50.0], memory_mb=10.0,
            latency_ms=1.0, hw_dict={},
        )
