...
def create_mock_model(
    ...
    model = WinMLModelForImageSegmentation.__new__(
        WinMLModelForImageSegmentation
    )
    mock_session = MagicMock()
    mock_session.run.return_value = {
        "logits": np.random.randn(1, num_queries, num_classes + 1).astype(
            np.float32
        ),
        "pred_boxes": np.random.randn(1, num_queries, 4).astype(np.float32),
        "pred_masks": np.random.randn(1, num_queries, output_h, output_w).astype(
            np.float32
        ),
    }
...
def create_mock_semantic_model(
    num_labels: int = 150, output_h: int = 128, output_w: int = 128
):
    ...
    model = WinMLModelForSemanticSegmentation.__new__(
        WinMLModelForSemanticSegmentation
    )
    mock_session = MagicMock()
    mock_session.run.return_value = {
        "logits": np.random.randn(1, num_labels, output_h, output_w).astype(
            np.float32
        )
    }
    mock_session.io_config = {
...
class TestWinMLModelForSemanticSegmentationBasic:
    ...
    def test_registered_in_task_mapping(self):
        ...
        assert (
            TASK_TO_WINML_CLASS["semantic-segmentation"]
            == "WinMLModelForSemanticSegmentation"
        )
