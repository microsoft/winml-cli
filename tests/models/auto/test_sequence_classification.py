...
def create_mock_model(num_labels: int = 2):
    ...
    model = WinMLModelForSequenceClassification.__new__(
        WinMLModelForSequenceClassification
    )
    mock_session = MagicMock()
    mock_session.run.return_value = {
        "logits": np.random.randn(1, num_labels).astype(np.float32)
    }
...
class TestWinMLModelForSequenceClassificationBasic:
    ...
    def test_inherits_from_base(self):
        ...
        assert issubclass(
            WinMLModelForSequenceClassification, WinMLPreTrainedModel
        )
