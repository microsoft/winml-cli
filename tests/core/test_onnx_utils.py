...
class TestGetIoConfig:
    ...
    def test_multiple_inputs_outputs(self) -> None:
        ...
        graph = helper.make_graph(
            [node1, node2], "test_graph", [in_a, in_b], [out_x, out_y]
        )
        model = helper.make_model(graph, opset_imports=[helper.make_opsetid("", 17)])
    ...
    def test_dynamic_dimensions(self) -> None:
        ...
        y_info = helper.make_tensor_value_info(
            "output", TensorProto.FLOAT, ["batch", 1000]
        )
    ...
    def test_various_dtypes(self) -> None:
        ...
        float32_input = helper.make_tensor_value_info(
            "float32_in", TensorProto.FLOAT, [1, 10]
        )
        int64_input = helper.make_tensor_value_info(
            "int64_in", TensorProto.INT64, [1, 10]
        )
        float16_output = helper.make_tensor_value_info(
            "float16_out", TensorProto.FLOAT16, [1, 10]
        )
        int32_output = helper.make_tensor_value_info(
            "int32_out", TensorProto.INT32, [1, 10]
        )
    ...
    def test_text_model_pattern(self) -> None:
        ...
        graph = helper.make_graph(
            [node], "encoder", [input_ids, attention_mask], [hidden_state]
        )
        model = helper.make_model(graph, opset_imports=[helper.make_opsetid("", 17)])
