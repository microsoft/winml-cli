...
class TestSaveOnnx:
    ...
    def test_save_external_falls_back_when_default_sidecar_is_locked(
        self, tmp_path: Path, monkeypatch: pytest.MonkeyPatch
    ) -> None:
        """Re-saving external data does not require deleting the old sidecar.

        On Windows, a just-loaded external-data sidecar can remain temporarily
        locked.  The default save path should still succeed by choosing a new
        sidecar name instead of failing while unlinking the old one.
        """
        model = _make_model_with_initializer()
        model_path = tmp_path / "export.onnx"
        save_onnx(model, model_path, threshold_size=0)

        default_data_path = tmp_path / "export.onnx.data"
        assert default_data_path.exists()

        loaded = load_onnx(model_path, validate=False)
        locked_path = default_data_path.resolve()
        path_cls = type(default_data_path)
        original_unlink = path_cls.unlink

        def locked_unlink(self, *args, **kwargs):  # type: ignore[no-untyped-def]
            if self.resolve() == locked_path:
        ...
        monkeypatch.setattr(path_cls, "unlink", locked_unlink)

        save_onnx(loaded, model_path, threshold_size=0)
        ...
        [location] = locations
        assert location != "export.onnx.data"
        assert default_data_path.exists()
        assert (tmp_path / location).exists()

        reloaded = load_onnx(model_path, validate=False)
        assert numpy_helper.to_array(reloaded.graph.initializer[0]).shape == (512,)

    def test_save_external_does_not_fallback_for_locked_explicit_location(
        self, tmp_path: Path, monkeypatch: pytest.MonkeyPatch
    ) -> None:
        """Caller-provided external data locations remain strict when locked."""
        model = _make_model_with_initializer()
        model_path = tmp_path / "export.onnx"
        save_onnx(model, model_path, threshold_size=0, location="weights.bin")

        explicit_data_path = tmp_path / "weights.bin"
        assert explicit_data_path.exists()

        loaded = load_onnx(model_path, validate=False)
        locked_path = explicit_data_path.resolve()
        path_cls = type(explicit_data_path)
        original_unlink = path_cls.unlink
        ...
        with pytest.raises(PermissionError):
            save_onnx(loaded, model_path, threshold_size=0, location="weights.bin")
