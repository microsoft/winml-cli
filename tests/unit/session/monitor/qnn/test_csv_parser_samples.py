...
def test_per_sample_retention_preserves_order() -> None:
    ...
    sample_1 = [
        {"name": "Conv2d", "op_path": "Conv2d", "op_id": 1, "cycles": 100},
        {"name": "Relu", "op_path": "Relu", "op_id": 2, "cycles": 5},
    ]
    sample_2 = [
        {"name": "Conv2d", "op_path": "Conv2d", "op_id": 1, "cycles": 110},
        {"name": "Relu", "op_path": "Relu", "op_id": 2, "cycles": 6},
    ]
...
def test_per_sample_back_compat_avg_cycles() -> None:
    ...
    sample_1 = [{"name": "X", "op_path": "X", "op_id": 7, "cycles": 100}]
    sample_2 = [{"name": "X", "op_path": "X", "op_id": 7, "cycles": 300}]
    ops = _aggregate_operators([sample_1, sample_2])
    assert len(ops) == 1
...
def test_per_sample_single_sample() -> None:
    ...
    ops = _aggregate_operators([[{"name": "X", "op_path": "X", "op_id": 1, "cycles": 42}]])
...
def test_per_sample_op_missing_in_some_samples() -> None:
    ...
    sample_1 = [
        {"name": "Conv2d", "op_path": "Conv2d", "op_id": 1, "cycles": 100},
        {"name": "OneShot", "op_path": "OneShot", "op_id": 99, "cycles": 7},
    ]
    sample_2 = [
        {"name": "Conv2d", "op_path": "Conv2d", "op_id": 1, "cycles": 200},
    ]
    ops = _aggregate_operators([sample_1, sample_2])
    by_id = {op["op_id"]: op for op in ops}
