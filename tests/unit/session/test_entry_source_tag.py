...
# --------------------------------------------------------------------------
"""Tests for ``_entry_source_tag`` 7-tag dispatch in ep_registry.

``_entry_source_tag`` maps an :class:`EPEntry`'s ``.source`` (an
:class:`EPSource` subclass instance) to one of seven canonical tag
strings used by :meth:`WinMLEPRegistry.auto_device` to filter cached
discovery entries by source.
"""
...
def _msix_microsoft() -> MSIXPackageSource:
...
@pytest.mark.parametrize(
    ...
    [
        ...
        (_msix_microsoft, "msix-microsoft"),
        (_msix_workload, "msix-workload"),
    ],
    ids=["pypi", "nuget", "winml-catalog", "directory", "msix-microsoft", "msix-workload"],
)
...
def test_msix_prefix_split_workload_vs_microsoft() -> None:
    """MSIXPackageSource dispatches on ``family_name_prefix`` substring.

    The ``WindowsWorkload.EP.`` OEM channel maps to ``"msix-workload"``;
    anything else (notably ``MicrosoftCorporationII.WinML.``) maps to
    ``"msix-microsoft"``.
    """
    ...
    microsoft = MSIXPackageSource(
        family_name_prefix="MicrosoftCorporationII.WinML.Qualcomm.QNN.EP.",
        relative_dll="ExecutionProvider/plugin.dll",
        eps=("QNNExecutionProvider",),
    )
    assert _entry_source_tag(_entry(workload)) == "msix-workload"
    assert _entry_source_tag(_entry(microsoft)) == "msix-microsoft"
