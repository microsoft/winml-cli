...
class TestSam2ReshapeTransposeReshapeMatching:
    ...
    def test_match_reshape_transpose_reshape_patterns(self, sam2_model):
        ...
        matcher.register_pattern(ReshapeTransposeReshapePattern())
    ...
    def test_no_merged_patterns_in_original_model(self, sam2_model):
        """Test that MergedReshapeTransposeReshape patterns are NOT found in original model.
        
        The original model has 6D transposes, not 4D merged transposes.
        """
        matcher = PatternMatcher(sam2_model, raise_on_invalid_model=False)
        matcher.register_pattern(MergedReshapeTransposeReshapePattern())

        results = matcher.match()

        assert len(results) == 0, \
            f"Expected 0 MergedReshapeTransposeReshape matches in original model, got {len(results)}"
    ...
    def test_matched_patterns_have_expected_structure(self, sam2_model):
        ...
        assert perm == (0, 1, 3, 2, 4, 5), \
            f"Expected perm (0, 1, 3, 2, 4, 5), got {perm}"
...
class TestSam2PatternRewriting:
    """Tests for rewriting ReshapeTransposeReshape to MergedReshapeTransposeReshape."""
    ...
    def test_rewrite_preserves_node_count(self, sam2_model):
        ...
        rewritten_model = rewriter.rewrite([
            (results, MergedReshapeTransposeReshapePattern),
        ])
    ...
    def test_rewritten_model_matches_merged_patterns(self, sam2_model):
        """Test that rewritten model contains MergedReshapeTransposeReshape patterns."""
        ...
        rewriter = PatternRewriter(sam2_model)
        rewritten_model = rewriter.rewrite([
            (original_results, MergedReshapeTransposeReshapePattern),
        ])

        # Match merged patterns in rewritten model
        matcher2 = PatternMatcher(rewritten_model, raise_on_invalid_model=False)
        matcher2.register_pattern(MergedReshapeTransposeReshapePattern())
        merged_results = matcher2.match()

        assert len(merged_results) == original_count, \
            f"Expected {original_count} MergedReshapeTransposeReshape matches, got {len(merged_results)}"

    def test_rewritten_model_still_matches_original_pattern(self, sam2_model):
        """Test that rewritten model also matches ReshapeTransposeReshape pattern.
        
        MergedReshapeTransposeReshape inherits from ReshapeTransposeReshapePattern,
        so the topology is the same (just with different shapes).
        """
        # Match original patterns
        matcher = PatternMatcher(sam2_model, raise_on_invalid_model=False)
        matcher.register_pattern(ReshapeTransposeReshapePattern())
        ...
        matcher2 = PatternMatcher(rewritten_model, raise_on_invalid_model=False)
        matcher2.register_pattern(ReshapeTransposeReshapePattern())
        rtr_results = matcher2.match()

        assert len(rtr_results) == original_count, \
            f"Expected {original_count} ReshapeTransposeReshape matches in rewritten model, got {len(rtr_results)}"
    ...
    def test_merged_patterns_have_reduced_dimensions(self, sam2_model):
        ...
        for i, result in enumerate(merged_results):
            ...
            assert len(transpose_shape) == 4, \
                f"Pattern {i}: Expected 4D merged transpose_shape, got {len(transpose_shape)}D: {transpose_shape}"
            assert len(perm) == 4, \
...
class TestPatternOverlapAssertion:
    ...
    def test_no_overlap_between_original_patterns(self, sam2_model):
        ...
        for result in results:
            ...
            assert not overlap, \
                f"Found overlapping nodes in pattern matches: {overlap}"
