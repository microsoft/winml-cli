...
def create_relu_model() -> onnx.ModelProto:
    ...
    """
    input_tensor = helper.make_tensor_value_info("input", TensorProto.FLOAT, [1, 64, 224, 224])
    output_tensor = helper.make_tensor_value_info("output", TensorProto.FLOAT, [1, 64, 224, 224])
...
def run_optimization_with_disabled_list(
    ...
    try:
        _ = ort.InferenceSession(str(input_file), sess_opts, providers=["CPUExecutionProvider"])
        success = True
    except Exception:
        success = False
...
class TestORTDisableLimit:
    ...
    def test_ort_accepts_empty_disable_list(self, temp_dir: Path) -> None:
        ...
        optimized, success = run_optimization_with_disabled_list(model, [], temp_dir)
    ...
    def test_ort_accepts_single_disabled_optimizer(self, temp_dir: Path) -> None:
        ...
        model = create_relu_model()
        optimized, success = run_optimization_with_disabled_list(model, ["GeluFusion"], temp_dir)
    ...
    def test_ort_accepts_10_disabled_optimizers(self, temp_dir: Path) -> None:
        ...
        optimized, success = run_optimization_with_disabled_list(model, optimizers, temp_dir)
    ...
    def test_character_limit_2048(self, temp_dir: Path) -> None:
        ...
        _optimized, success = run_optimization_with_disabled_list(model, fake_optimizers, temp_dir)
    ...
    def test_ort_actually_disables_specified_optimizers(self, temp_dir: Path) -> None:
        ...
        optimized_full, _ = run_optimization_with_disabled_list(model, [], temp_dir)
        ...
        optimized_disabled, _ = run_optimization_with_disabled_list(model, all_optimizers, temp_dir)
    ...
    def test_incremental_limit_discovery(self, temp_dir: Path) -> None:
        ...
        for count in [10, 20, 30, 32, 35, 40, 45, 50]:
            ...
            _, success = run_optimization_with_disabled_list(model, subset, temp_dir)
