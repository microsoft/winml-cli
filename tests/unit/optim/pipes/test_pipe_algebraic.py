...
# --------------------------------------------------------------------------
"""Generated-graph tests for exact algebraic optimizer rewrites."""
...
class TestAlgebraicRegistration:
    ...
    def test_capabilities_are_opt_in_and_independent(self) -> None:
        ...
        assert config.conv_add_batch_normalization_folding is False

    def test_cli_lists_algebraic_flags(self) -> None:
...
class TestConvChannelAffineFolding:
    ...
    def affine_model(self) -> tuple[onnx.ModelProto, dict[str, np.ndarray]]:
        ...
        nodes = [
            helper.make_node("Conv", ["x", "weights"], ["conv_out"], name=""),
            helper.make_node("Mul", ["conv_out", "scale"], ["mul_out"], name=""),
            helper.make_node("Add", ["offset", "mul_out"], ["y"], name=""),
        ]
    ...
    def test_shared_conv_output_is_ineligible(
        ...
        model.graph.node.append(helper.make_node("Identity", ["conv_out"], ["other"]))
    ...
    def test_routed_view_graph_output_is_ineligible(self) -> None:
        ...
        model = _model(
            [
                helper.make_node("Conv", ["x", "weight"], ["conv_out"]),
                helper.make_node("Split", ["conv_out"], ["left", "right"], axis=1),
                helper.make_node("Reshape", ["left", "view_shape"], ["left_view"]),
                helper.make_node("Mul", ["left_view", "scale"], ["left_scaled"]),
                helper.make_node("Identity", ["right"], ["right_out"]),
            ],
            [_info("x", [1, 1, 2, 2])],
            [
    ...
    def test_static_split_branches_fold_without_overlapping_ranges(self) -> None:
        ...
        nodes = [
            helper.make_node("Conv", ["x", "weights"], ["conv_out"]),
            helper.make_node(
                "Split",
                ["conv_out", "sizes"],
                ["first", "second"],
            ...
            helper.make_node("Mul", ["first", "scale_first"], ["first_scaled"]),
            helper.make_node("Add", ["first_scaled", "offset_first"], ["first_out"]),
            helper.make_node("Add", ["second", "offset_second"], ["second_out"]),
        ]
        model = _model(
            nodes,
            [x],
    ...
    def test_channel_preserving_views_and_nested_slices_fold(self) -> None:
        ...
        nodes = [
            helper.make_node("Conv", ["x", "weights"], ["conv_out"]),
            helper.make_node("Reshape", ["conv_out", "view_shape"], ["viewed"]),
            helper.make_node("Split", ["viewed", "split_sizes"], ["first", "second"], axis=1),
            helper.make_node("Squeeze", ["first", "squeeze_axes"], ["first_view"]),
            helper.make_node("Mul", ["first_view", "first_scale"], ["first_scaled"]),
            helper.make_node("Relu", ["first_scaled"], ["first_out"]),
            helper.make_node(
                "Slice",
            ...
            helper.make_node("Mul", ["second_a", "second_a_scale"], ["second_a_out"]),
            helper.make_node("Add", ["second_b", "second_b_offset"], ["second_b_out"]),
        ]
        model = _model(
            nodes,
            [x],
    ...
    def test_multiple_independent_affine_matches_fold(self) -> None:
        ...
        nodes = [
            helper.make_node("Conv", ["x1", "weight1"], ["conv1"]),
            helper.make_node("Mul", ["conv1", "scale1"], ["y1"]),
            helper.make_node("Conv", ["x2", "weight2"], ["conv2"]),
            helper.make_node("Add", ["conv2", "offset2"], ["y2"]),
        ]
        model = _model(
            nodes,
    ...
    def test_nested_subgraph_captures_make_affine_fold_ineligible(
        ...
        then_branch = helper.make_graph(
            [helper.make_node("Identity", ["mul_out"], ["then_output"])],
            "then_branch",
            [],
            [_info("then_output", branch_shape)],
        )
        else_branch = helper.make_graph(
            [helper.make_node("Identity", ["conv_out"], ["else_output"])],
            "else_branch",
            [],
    ...
    def test_constant_attribute_affine_is_folded_and_pruned(self) -> None:
        ...
        model = _model(
            [
                helper.make_node("Conv", ["x", "weight"], ["conv_out"]),
                helper.make_node("Constant", [], ["scale"], value_float=1.5),
                helper.make_node("Mul", ["conv_out", "scale"], ["y"]),
            ],
            [_info("x", [1, 1, 2, 2])],
            [_info("y", [1, 1, 2, 2])],
...
class TestConvAddBatchNormalizationFolding:
    """Test inference-only Conv/Add/BatchNormalization folding."""

    def _build(
        self, *, training_input: bool = False, dynamic_params: bool = False
    ) -> tuple[onnx.ModelProto, dict[str, np.ndarray]]:
        rng = np.random.default_rng(13)
        x = helper.make_tensor_value_info("x", TensorProto.FLOAT, [1, 2, 2, 2])
        params = {
            "weights": rng.normal(size=(3, 2, 1, 1)).astype(np.float32),
            "add_tensor": rng.normal(size=(1, 3, 1, 1)).astype(np.float32),
            ...
            "variance": rng.uniform(0.5, 1.5, size=3).astype(np.float32),
        }
        nodes = [
            helper.make_node("Conv", ["x", "weights"], ["conv_out"]),
            helper.make_node("Add", ["conv_out", "add_tensor"], ["add_out"]),
        ]
        bn_inputs = ["add_out", "scale", "beta", "mean", "variance"]
        inputs = [x]
        if dynamic_params:
            inputs.append(helper.make_tensor_value_info("scale", TensorProto.FLOAT, [3]))
            bn_inputs[1] = "scale"
            params.pop("scale")
        training_attribute = 1 if training_input else None
        bn_attributes = {"epsilon": 0.01}
        if training_attribute is not None:
            bn_attributes["training_mode"] = training_attribute
        nodes.append(
            helper.make_node(
                "BatchNormalization",
                bn_inputs,
        ...
        model = _model(
            ...
            inputs,
            [_info("y", [1, 3, 2, 2])],
            [_tensor(name, value) for name, value in params.items()],
            value_info=[
                _info("conv_out", [1, 3, 2, 2]),
                _info("add_out", [1, 3, 2, 2]),
            ],
        )
        return model, {"x": rng.normal(size=(1, 2, 2, 2)).astype(np.float32)}

    def test_inference_fold_scales_copied_add_tensor(self) -> None:
        model, values = self._build()
        # Make the Add tensor shared, proving that the source initializer is not
        # mutated in place by the rewrite.
        model.graph.node.append(helper.make_node("Identity", ["add_tensor"], ["kept_tensor"]))
        model.graph.output.append(_info("kept_tensor", [1, 3, 1, 1]))
        original_add = next(
            initializer
            for initializer in model.graph.initializer
            if initializer.name == "add_tensor"
        )
        original_values = np.array(numpy_helper.to_array(original_add), copy=True)
        transformed = AlgebraicRewritePipe().process(
            model,
            AlgebraicRewritePipeConfig(conv_add_batch_normalization_folding=True),
        )
        assert [node.op_type for node in transformed.graph.node] == ["Conv", "Add", "Identity"]
        ...
        np.testing.assert_array_equal(numpy_helper.to_array(original_add), original_values)
        used_initializers = {
            input_name for node in transformed.graph.node for input_name in node.input if input_name
        }
        assert {
            initializer.name for initializer in transformed.graph.initializer
        } <= used_initializers
        ...
        np.testing.assert_allclose(
            _run(model, values)[0],
            _run(transformed, values)[0],
            rtol=3e-5,
            atol=3e-5,
        )

    def test_training_mode_and_dynamic_parameters_are_skipped(self) -> None:
        for training_input, dynamic_params in ((True, False), (False, True)):
            model, _ = self._build(
    ...
    def test_public_conv_output_makes_fold_ineligible(self) -> None:
        model, values = self._build()
        model.graph.output.append(_info("conv_out", [1, 3, 2, 2]))
        transformed = AlgebraicRewritePipe().process(
            model,
            AlgebraicRewritePipeConfig(conv_add_batch_normalization_folding=True),
        )
    ...
    def test_multiple_independent_batch_normalizations_fold(self) -> None:
        ...
        for suffix in ("a", "b"):
            input_name = f"x_{suffix}"
            conv_output = f"conv_{suffix}"
            add_output = f"add_{suffix}"
            output_name = f"y_{suffix}"
            inputs.append(_info(input_name, [1, 1, 2, 2]))
            outputs.append(_info(output_name, [1, 1, 2, 2]))
            value_info.extend(
                [
                    _info(conv_output, [1, 1, 2, 2]),
                    _info(add_output, [1, 1, 2, 2]),
                ]
            )
            nodes.extend(
                [
                    helper.make_node(
                        "Conv",
                        [input_name, f"weight_{suffix}"],
                        [conv_output],
                    ),
                    helper.make_node(
                        "Add",
                        [conv_output, f"static_{suffix}"],
                        [add_output],
                    ),
                    helper.make_node(
                        "BatchNormalization",
            ...
            initializers.extend(
                [
                    _tensor(
                        f"weight_{suffix}",
                        rng.normal(size=(1, 1, 1, 1)).astype(np.float32),
                    ),
                    _tensor(
                        f"static_{suffix}",
                        rng.normal(size=(1, 1, 2, 2)).astype(np.float32),
                    ),
                    _tensor(f"scale_{suffix}", np.asarray([1.25], dtype=np.float32)),
                    _tensor(f"beta_{suffix}", np.asarray([0.1], dtype=np.float32)),
                    _tensor(f"mean_{suffix}", np.asarray([-0.2], dtype=np.float32)),
                    _tensor(f"variance_{suffix}", np.asarray([0.75], dtype=np.float32)),
                ]
            )
            values[input_name] = rng.normal(size=(1, 1, 2, 2)).astype(np.float32)
        ...
        for original, rewritten in zip(
            ...
            np.testing.assert_allclose(original, rewritten, rtol=3e-5, atol=3e-5)
