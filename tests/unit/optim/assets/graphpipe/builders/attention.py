...
def attention_builder():
    ...
    nodes.append(helper.make_node("MatMul", ["ln_out", "wq"], ["q_matmul"], name="q_matmul"))
    ...
    nodes.append(helper.make_node("Reshape", ["q_add", "q_shape"], ["q_reshape"], name="q_reshape"))
    nodes.append(
        helper.make_node(
            "Transpose",
            ["q_reshape"],
    ...
    nodes.append(helper.make_node("MatMul", ["ln_out", "wk"], ["k_matmul"], name="k_matmul"))
    ...
    nodes.append(helper.make_node("Reshape", ["k_add", "k_shape"], ["k_reshape"], name="k_reshape"))
    nodes.append(
        helper.make_node(
            "Transpose",
            ["k_reshape"],
    ...
    nodes.append(helper.make_node("MatMul", ["ln_out", "wv"], ["v_matmul"], name="v_matmul"))
    ...
    nodes.append(helper.make_node("Reshape", ["v_add", "v_shape"], ["v_reshape"], name="v_reshape"))
    nodes.append(
        helper.make_node(
            "Transpose",
            ["v_reshape"],
    ...
    nodes.append(helper.make_node("MatMul", ["q_trans", "k_trans"], ["qk"], name="qk_matmul"))
    ...
    nodes.append(
        helper.make_node("Add", ["qk_scaled", "attention_mask"], ["qk_masked"], name="mask_add")
    )
    nodes.append(helper.make_node("Softmax", ["qk_masked"], ["attn_probs"], name="softmax", axis=3))
    nodes.append(helper.make_node("MatMul", ["attn_probs", "v_trans"], ["attn_out"], name="attn_v"))
    ...
    nodes.append(
        helper.make_node(
            ...
            outputs=["out_shape"],
            name="out_shape",
            value=helper.make_tensor("out_shape", TensorProto.INT64, [3], [0, 0, hidden_size]),
        )
    )
    nodes.append(
        helper.make_node("Reshape", ["out_trans", "out_shape"], ["out_reshape"], name="out_reshape")
    )
    nodes.append(
        helper.make_node("MatMul", ["out_reshape", "wo"], ["out_matmul"], name="out_matmul")
    )
    nodes.append(helper.make_node("Add", ["out_matmul", "bo"], ["out_add"], name="out_add"))
    # Skip connection: add back ln_out (this is the 4th edge from LayerNorm output)
    nodes.append(helper.make_node("Add", ["out_add", "ln_out"], ["output"], name="skip_add"))
    ...
    input_2 = helper.make_tensor_value_info("input_2", TensorProto.INT32, [batch_size, seq_len])
...
def multi_head_attention_builder(
    ...
    # Scale factor
    initializers.append(
        numpy_helper.from_array(np.array([1.0 / math.sqrt(16)], dtype=np.float32), f"{prefix}scale")
    )
...
def rotary_embeddings_builder(
    ...
    initializers.append(
        numpy_helper.from_array(rng.uniform(0.8, 1.0, 64).astype(np.float32), f"{prefix}cos_pos")
    )
    initializers.append(
        numpy_helper.from_array(rng.uniform(-0.2, 0.2, 64).astype(np.float32), f"{prefix}sin_pos")
    )
