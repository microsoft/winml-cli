...
def conv_bn_builder(input_name: str, output_name: str, prefix: str, initializers: list) -> list:
    ...
    initializers.append(numpy_helper.from_array(np.ones(16, dtype=np.float32), f"{prefix}bn_scale"))
    initializers.append(numpy_helper.from_array(np.zeros(16, dtype=np.float32), f"{prefix}bn_bias"))
    initializers.append(numpy_helper.from_array(np.zeros(16, dtype=np.float32), f"{prefix}bn_mean"))
    initializers.append(numpy_helper.from_array(np.ones(16, dtype=np.float32), f"{prefix}bn_var"))
...
def conv_add_relu_builder(
    ...
    initializers.append(
        numpy_helper.from_array(rng.randn(16, 1, 1).astype(np.float32) * 0.1, f"{prefix}bias")
    )
    ...
    return [
        ...
        helper.make_node("Relu", [f"{prefix}add2_out"], [output_name], name=f"{prefix}relu"),
    ]
...
def conv_activation_builder(
    ...
    initializers.append(
        numpy_helper.from_array(rng.randn(16).astype(np.float32) * 0.1, f"{prefix}conv_b")
    )
...
def conv_mul_builder(input_name: str, output_name: str, prefix: str, initializers: list) -> list:
...
def pad_conv_builder(input_name: str, output_name: str, prefix: str, initializers: list) -> list:
    ...
    initializers.append(
        numpy_helper.from_array(np.array([1, 1, 8, 8], dtype=np.int64), f"{prefix}reshape_shape")
    )
    ...
    initializers.append(
        numpy_helper.from_array(rng.randn(1, 1, 3, 3).astype(np.float32) * 0.1, f"{prefix}conv_w")
    )
    # Reshape back to [1, 64]
    initializers.append(
        numpy_helper.from_array(np.array([1, 64], dtype=np.int64), f"{prefix}out_shape")
