...
def bert_attention_builder(
    ...
    nodes.append(
        helper.make_node(
            "MatMul",
            [f"{prefix}ln_out", f"{prefix}q_weight"],
            [f"{prefix}q_mm"],
    ...
    nodes.append(
        helper.make_node(
            "Reshape",
            [f"{prefix}q_add", f"{prefix}reshape_qk"],
            [f"{prefix}q_reshape"],
    ...
    nodes.append(
        helper.make_node(
            "MatMul",
            [f"{prefix}ln_out", f"{prefix}k_weight"],
            [f"{prefix}k_mm"],
    ...
    nodes.append(
        helper.make_node(
            "Reshape",
            [f"{prefix}k_add", f"{prefix}reshape_qk"],
            [f"{prefix}k_reshape"],
    ...
    nodes.append(
        helper.make_node(
            "MatMul",
            [f"{prefix}ln_out", f"{prefix}v_weight"],
            [f"{prefix}v_mm"],
    ...
    nodes.append(
        helper.make_node(
            "Reshape",
            [f"{prefix}v_add", f"{prefix}reshape_qk"],
            [f"{prefix}v_reshape"],
    ...
    nodes.append(
        helper.make_node(
            "Unsqueeze",
            [f"{prefix}mask_unsq1", f"{prefix}axes_2"],
            [f"{prefix}mask_unsq2"],
    ...
    nodes.append(
        helper.make_node(
            "Sub",
            [f"{prefix}sub_weight", f"{prefix}mask_cast"],
            [f"{prefix}mask_sub"],
    ...
    nodes.append(
        helper.make_node(
            "Mul",
            [f"{prefix}mask_sub", f"{prefix}mul_weight"],
            [f"{prefix}mask_out"],
    ...
    nodes.append(
        helper.make_node(
            "MatMul",
            [f"{prefix}q_trans", f"{prefix}k_trans"],
            [f"{prefix}qk_mm"],
    ...
    nodes.append(
        helper.make_node(
            "Div",
            [f"{prefix}qk_mm", f"{prefix}div_weight"],
            [f"{prefix}qk_div"],
    ...
    nodes.append(
        helper.make_node(
            "Add",
            [f"{prefix}qk_div", f"{prefix}mask_out"],
            [f"{prefix}qk_add"],
    ...
    nodes.append(
        helper.make_node(
            "MatMul",
            [f"{prefix}attn_weights", f"{prefix}v_trans"],
            [f"{prefix}attn_v"],
    ...
    nodes.append(
        helper.make_node(
            "Reshape",
            [f"{prefix}attn_trans", f"{prefix}reshape_out"],
            [f"{prefix}attn_reshape"],
    ...
    nodes.append(
        helper.make_node(
            "MatMul",
            [f"{prefix}attn_reshape", f"{prefix}out_weight"],
            [f"{prefix}out_mm"],
    ...
    nodes.append(
        helper.make_node(
            "Add",
            [f"{prefix}out_mm", f"{prefix}out_bias"],
            [f"{prefix}out_add"],
    ...
    nodes.append(
        helper.make_node(
            "Add",
            [f"{prefix}out_add", f"{prefix}ln_out"],
            [f"{prefix}skip_out"],
...
def create_bert_attention_model(
    ...
    mask = helper.make_tensor_value_info("attention_mask", TensorProto.INT64, [batch_size, seq_len])
...
def gpt2_attention_builder(
    ...
    reshape_shape = helper.make_tensor(
        f"{prefix}reshape_shape",
        TensorProto.INT64,
        [4],
        np.array([0, 0, num_heads, head_size], dtype=np.int64),
    )
    reshape_back = helper.make_tensor(
        f"{prefix}reshape_back",
        TensorProto.INT64,
        [3],
        np.array([0, 0, hidden_size], dtype=np.int64),
    )
    ...
    nodes.append(
        helper.make_node(
            "MatMul",
            [input_name, f"{prefix}qkv_weight"],
            [f"{prefix}qkv_matmul"],
    ...
    nodes.append(
        helper.make_node(
            "Add",
            [f"{prefix}qkv_matmul", f"{prefix}qkv_bias"],
            [f"{prefix}qkv_out"],
    ...
    nodes.append(
        helper.make_node(
            "Split",
            [f"{prefix}qkv_out", f"{prefix}split_sizes"],
    ...
    nodes.extend(
        helper.make_node(
            "Reshape",
            [f"{prefix}{proj}_split", f"{prefix}reshape_shape"],
            [f"{prefix}{proj}_reshaped"],
            name=f"{prefix}{proj}_reshape",
            allowzero=0,
        )
        for proj in ["q", "k", "v"]
    )
    ...
    nodes.extend(
        helper.make_node(
            "Transpose",
            [f"{prefix}{proj}_reshaped"],
            [f"{prefix}{proj}_transposed"],
            name=f"{prefix}{proj}_transpose",
            perm=[0, 2, 1, 3],
        )
        for proj in ["q", "v"]
    )
    ...
    nodes.append(
        helper.make_node(
            "MatMul",
            [f"{prefix}q_transposed", f"{prefix}k_transposed"],
            [f"{prefix}qk"],
    ...
    nodes.append(
        helper.make_node(
            "MatMul",
            [f"{prefix}attn_weights", f"{prefix}v_transposed"],
            [f"{prefix}attn_out"],
    ...
    nodes.append(
        helper.make_node(
            "Transpose",
            [f"{prefix}attn_out"],
            [f"{prefix}attn_transposed"],
            name=f"{prefix}attn_transpose",
            perm=[0, 2, 1, 3],
        )
    )
    ...
    nodes.append(
        helper.make_node(
            "Reshape",
            [f"{prefix}attn_transposed", f"{prefix}reshape_back"],
            [f"{prefix}attn_reshaped"],
            name=f"{prefix}attn_reshape_back",
            allowzero=0,
        )
    )
    ...
    nodes.append(
        helper.make_node(
            "MatMul",
            [f"{prefix}attn_reshaped", f"{prefix}out_weight"],
            [f"{prefix}out_matmul"],
    ...
    nodes.append(
        helper.make_node(
            "Add",
            [f"{prefix}out_matmul", f"{prefix}out_bias"],
            [output_name],
