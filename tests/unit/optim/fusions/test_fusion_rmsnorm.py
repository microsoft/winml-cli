...
def _make_rmsnorm_model(
    ...
    reduce_mean = helper.make_node("ReduceMean", ["pow_out"], ["mean_out"], axes=[-1], keepdims=1)
...
def _make_multi_rmsnorm_model(
    ...
    x_info = helper.make_tensor_value_info("input", TensorProto.FLOAT, [1, 8, hidden_size])
    ...
    for i in range(num_layers):
        ...
        pow_exp = numpy_helper.from_array(np.array(2.0, dtype=np.float32), f"{prefix}pow_exp")
        epsilon = numpy_helper.from_array(np.array(1e-6, dtype=np.float32), f"{prefix}epsilon")
        weight = numpy_helper.from_array(weight_val, f"{prefix}weight")
        initializers.extend([pow_exp, epsilon, weight])

        pow_node = helper.make_node("Pow", [prev_output, f"{prefix}pow_exp"], [f"{prefix}pow_out"])
        ...
        sqrt_node = helper.make_node("Sqrt", [f"{prefix}add_out"], [f"{prefix}sqrt_out"])
        div_node = helper.make_node("Div", [prev_output, f"{prefix}sqrt_out"], [f"{prefix}div_out"])
        mul_weight = helper.make_node("Mul", [f"{prefix}div_out", f"{prefix}weight"], [out_name])
        nodes.extend([pow_node, reduce_mean, add_eps, sqrt_node, div_node, mul_weight])
        prev_output = out_name

    y_info = helper.make_tensor_value_info(prev_output, TensorProto.FLOAT, [1, 8, hidden_size])
...
class TestWeightAdjustment:
    ...
    def test_weight_adjustment(self):
        ...
        adjusted_inits = [
            init for init in result.graph.initializer if "l2norm_adjusted" in init.name
        ]
    ...
    def test_all_ones_weight_collapses(self):
        ...
        model = _make_rmsnorm_model(hidden_size=hidden_size, weight_value=ones_weight)
        result = _apply_fusion(model)
...
class TestNoMatch:
    ...
    def test_no_match_without_weight_initializer(self):
        ...
        w_info = helper.make_tensor_value_info("weight_runtime", TensorProto.FLOAT, [hidden_size])
        y_info = helper.make_tensor_value_info("output", TensorProto.FLOAT, [1, 8, hidden_size])
