...
class TestAudioLabelAlignmentAndSampling:
    def test_fleurs_classlabel_maps_de_de_to_model_id_55(self):
        encoded = BytesIO()
        sf.write(encoded, np.ones(8, dtype=np.float32), 16_000, format="WAV")
        language_names = [f"language_{index}" for index in range(17)] + ["de_de"]
        dataset = Dataset.from_list(
            [
                {
                    "audio": {"bytes": encoded.getvalue(), "path": "german.wav"},
                    "lang_id": 17,
                    "language": "German",
                }
            ],
            features=Features(
                {
                    ...
                    "lang_id": ClassLabel(names=language_names),
                    "language": Value("string"),
                }
            ),
        )
        id2label = {index: f"model_{index}" for index in range(56)}
        id2label[55] = "deu"

        class _LanguageClassifier:
            io_config: ClassVar = {
                "input_names": ["input_values"],
                "input_shapes": [[1, 8]],
                "output_names": ["logits"],
        ...
        config = WinMLEvaluationConfig(
            ...
            dataset=DatasetConfig(
                ...
                samples=1,
                shuffle=False,
                columns_mapping={"label_column": "lang_id"},
                label_mapping={"de_de": 55},
        ...
        with (
            ...
            evaluator = WinMLAudioClassificationEvaluator(config, _LanguageClassifier())
            metrics = evaluator.compute()

        assert isinstance(evaluator._label_feature, ClassLabel)
        assert evaluator._label_feature.int2str(17) == "de_de"
        assert evaluator.data[0].model_id == 55
        assert metrics["accuracy"] == 1.0
        assert metrics["macro_f1"] == 1.0
        assert metrics["processed_samples"] == 1
        assert metrics["inference_windows"] == 1
        assert metrics["per_label_processed"] == {"deu": 1}
