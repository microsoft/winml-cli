...
class TestFixedShapePipeline:
    """The subclass must force ``padding='max_length'`` regardless of caller."""

    def test_overrides_padding_to_max_length(self) -> None:
        ...
        with patch.object(ZeroShotClassificationPipeline, "_parse_and_tokenize", _fake_super_parse):
            instance._parse_and_tokenize([("a", "b")], padding=True)

        assert captured["padding"] == "max_length"
