...
def sequence_classification_onnx(tmp_path: Path) -> Path:
    ...
    # FC weights
    fc_w = helper.make_tensor(
        "fc_w", TensorProto.FLOAT, [64, 2], np.random.randn(64, 2).astype(np.float32).flatten()
    )
...
requires_hf_hub = pytest.mark.skipif(
    os.environ.get("WINML_TEST_OFFLINE", "0") == "1", reason="HF Hub tests disabled in offline mode"
)
