...
def test_mgp_str_stale_architecture_uses_image_to_text_pipeline_tag(monkeypatch):
    ...
    result = resolve_task(cfg)

    assert result.task == "image-to-text"
    assert result.model_class.__name__ == "MgpstrForSceneTextRecognition"
    assert result.source == TaskSource.PIPELINE_TAG
