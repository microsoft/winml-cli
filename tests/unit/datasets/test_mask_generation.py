...
def _build_fake_ds(
    ...
    *,
    ...
    for i in range(n_rows):
        ...
        mask[10:22, 14:24] = 1
        row = {image_col: _make_pil(img), mask_col: _make_pil(mask)}
        if with_text:
...
class TestValidation:
    ...
    def test_explicit_image_and_mask_columns_override_heuristics(self, monkeypatch):
        fake = _build_fake_ds(n_rows=1, image_col="rgb", mask_col="truth")
        ...
        ds = _make_ds(
            ...
            max_mask_coverage=1.0,
        )

        assert ds.image_col == "truth"
        assert ds.mask_col == "rgb"

    def test_explicit_single_column_override_keeps_other_column_auto_detected(self, monkeypatch):
        fake = _build_fake_ds(n_rows=1, image_col="rgb", mask_col="mask")
        ...
        assert ds.image_col == "rgb"
        assert ds.mask_col == "mask"

    def test_load_dataset_receives_name_revision_and_streaming(self, monkeypatch):
        fake = _build_fake_ds(n_rows=1)
    ...
    def test_streaming_is_rejected_with_clear_error(self, monkeypatch):
        fake = _build_fake_ds(n_rows=1)
        ...
        with (
            ...
            pytest.raises(ValueError, match="streaming=True"),
        ):
            MaskGenerationDataset(
                model_name="dummy/model",
                dataset_name="dummy/dataset",
