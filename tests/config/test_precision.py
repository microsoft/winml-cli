...
class TestResolvePrecision:
    ...
    def test_auto_device_picks_best(
        ...
        policy = resolve_precision(
            device="auto", precision=precision, available_devices=available,
        )
        assert policy.device == exp_device
...
class TestResolveQuantTypes:
    ...
    def test_named_presets(
        self, precision: str, exp_weight: str, exp_act: str
    ) -> None:
...
class TestIsQuantizedPrecision:
    ...
    @pytest.mark.parametrize(
        "precision", ["garbage", "wXaY", "", "bfloat16", "w0a0"]
    )
...
class TestQuantizeCliResolveQuant:
    ...
    def test_both_explicit_override_precision(self) -> None:
        ...
        w, a = self._resolve(
            precision="w8a16", weight_type="int8", activation_type="int16"
        )
        assert w == "int8"
        assert a == "int16"
