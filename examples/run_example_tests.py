...
# The only scope for this runner: the centralized recipe set under
# examples/recipes/<model_slug>/<task>_<precision>_config.json.
RECIPES_DIR = REPO_ROOT / "examples" / "recipes"
# Build artifacts go here so they stay out of git.
BUILD_ARTIFACTS_ROOT = REPO_ROOT / "temp" / "example_builds"
# Scope is defined by the registry: only recipes for these tasks are touched.
REGISTRY_PATH = REPO_ROOT / "scripts" / "e2e_eval" / "testsets" / "models_with_acc.json"
DEFAULT_TASKS = ("depth-estimation", "keypoint-detection")
...
def load_target_slugs(tasks: set[str]) -> set[str]:
    """Recipe dir slugs for registry entries whose task is in ``tasks``.

    Defines the scope: only these models are built/evaluated, leaving every
    other recipe under examples/recipes/ untouched.
    """
    try:
    ...
    return {
        e["hf_id"].replace("/", "_")
        for e in entries
        if isinstance(e, dict) and e.get("task") in tasks and e.get("hf_id")
    }
...
def canonical_ep_device_columns() -> list[str]:
    """Every ``<ep_folder>/<device>`` pair winml targets, sorted.

    Derived from ``EP_SUPPORTED_DEVICES`` (the single source of truth) using the
    same examples-folder naming as :func:`resolve_ep_and_examples_folder`
    (e.g. the CPU EP maps to the ``mlas`` folder). CUDA is excluded to match
    the runner's ``EP_CHOICES``. Used by the report to show the full coverage
    grid even before any run exists.
    """
    cols: list[str] = []
    for ep_name, devices in EP_SUPPORTED_DEVICES.items():
        alias = EP_NAME_TO_ALIAS[ep_name]
        if alias in ("cuda",):
            continue
        folder = EP_TO_EXAMPLES_FOLDER.get(alias, alias)
        cols.extend(f"{folder}/{device}" for device in devices)
    return sorted(cols)
...
def _build_error_path(marker_dir: Path, stem: str) -> Path:
    return marker_dir / f"{stem}_build_result.error.txt"


def _build_timeout_path(marker_dir: Path, stem: str) -> Path:
    return marker_dir / f"{stem}_build_result.timeout"
...
def run_build(
    ...
    err_path = _build_error_path(marker_dir, stem)
    timeout_path = _build_timeout_path(marker_dir, stem)
...
def main() -> None:
    ...
    parser.add_argument(
        ...
        help=(
            ...
            f"(default: {', '.join(DEFAULT_TASKS)}). Only registry models with "
            "these tasks are built/evaluated; all other recipes are ignored."
        ),
    )
    parser.add_argument(
        "--precision",
    ...
    # Results are git-tracked under examples/<ep>/<device>/<model>/ so the
    # print_example_result.py matrix can read them; builds go to temp/.
    result_root = REPO_ROOT / "examples" / examples_ep / args.device
    build_root = BUILD_ARTIFACTS_ROOT / examples_ep / args.device

    # Device drives which precisions run: NPU covers fp16 + quantized;
    # CPU/GPU run fp16 only. An explicit --precision still overrides this.
    if args.precision is not None:
        precision_filter: str | None = args.precision
    elif args.device == "npu":
        precision_filter = None
    else:
        precision_filter = "fp16"

    model_dirs = sorted(d for d in RECIPES_DIR.iterdir() if d.is_dir())
    target_tasks = {t.strip() for t in args.tasks.split(",") if t.strip()}
    target_slugs = load_target_slugs(target_tasks)
    model_dirs = [d for d in model_dirs if d.name in target_slugs]
    if args.models:
        allowed = set(args.models.split(","))
        model_dirs = [d for d in model_dirs if d.name in allowed]

    grouped_configs = build_grouped_configs(model_dirs, precision=precision_filter)

    print(
        f"EP: {ep_for_winml}, Device: {args.device} -> "
        f"results in examples/{examples_ep}/{args.device}"
    )
    print(
        f"Models: {len(model_dirs)}, Config groups: {len(grouped_configs)} "
        f"(precision: {precision_filter or 'all'})"
    )
    print()
    ...
    for i, (model_dir, group_stem, group_paths) in enumerate(grouped_configs, 1):
        ...
        eval_output = result_dir / f"{group_stem}_eval_result.json"
        ...
        group_build_root = build_root / model_slug / group_stem
