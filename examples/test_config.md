...
Run eval for all configs under a given EP:
...
2. Runs `winml eval -m <hf_id> --device <device> -c <config> -o <eval_output>` for each
...
├── image-classification_w8a8_eval.json     # success
...
# Model with dataset_script (adds --trust-remote-code)
...
`winml eval` automatically builds the model (export → optimize → quantize → compile) if no artifacts exist.
