...
specialized` switches `winml build` from the stock per-model ONNX output to a
...
`--export-type specialized` **infers** the recipe's target (Qwen3 builds for the
NPU HTP via QNN), so `--ep`/`--device` are optional — and, unlike the shortcut
below, the specialized path does not gate on detecting the target device locally.
...
!!! note "Backward-compatible shortcut"
    ...
    the NPU) still routes to its specialized bundle:
...
- **Genai bundles require `--output-dir`, not `--use-cache`.** A specialized
  decoder-LLM bundle (`--export-type specialized`, or the `--device npu --ep qnn`
