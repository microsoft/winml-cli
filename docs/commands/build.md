...
CPU, QNN/NPU, and VitisAI/NPU. Other targets fail fast (naming the `ep`/`device`)
instead of silently producing a generic model. Pin the provider when needed:
...
# CPU, without an NPU provider
winml build -m Qwen/Qwen3-0.6B -o out/qwen3-bundle --export-type optimized --ep cpu --device cpu
```
