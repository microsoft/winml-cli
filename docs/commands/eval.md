...
| `--export / --no-export` | | flag | `export` | Export a Hugging Face checkpoint to ONNX before evaluation. `--no-export` evaluates the original PyTorch checkpoint and supports `auto`, `cpu`, or CUDA-backed `gpu` devices. |
...
```bash
$ winml eval -m microsoft/resnet-50 --no-export --device cpu
```
...
- **Native evaluation accepts only Hugging Face checkpoints.** `--no-export` cannot be combined with ONNX files, composite models, GenAI bundles, compare/reference/input-data modes, EP selection, or ONNX build/export controls. Use `--device cpu`, `--device gpu` with CUDA, or `--device auto`.
