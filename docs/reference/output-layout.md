...
| `build_manifest.json` | Build provenance with stage timings. Generated by `winml build` and the Python API (`build_hf_model`/`build_onnx_model`). `winml export` writes `export_manifest.json` instead. |
