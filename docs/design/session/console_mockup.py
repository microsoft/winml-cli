...
class SourceRow:
    ...
    status: Status
    kind_label: str             # "PyPI" / "NuGet" / "MSIX" / "Catalog" / "Directory" / "built-in"
    source_label: str | None    # distribution / family name; None for built-in
    version: str | None         # package / distribution version; None for built-in
    catalog_default: bool       # true → render "(catalog default)" annotation
    path: str | None            # None for built-in
...
class EPBlock:
    ...
    name: str
    device_summary: str         # e.g. "Intel NPU/GPU/CPU" / "GPU" / "CPU"
...
MOCK_LISTING: Final[MockListing] = MockListing(
    ...
    ep_blocks=(
        EPBlock(
            ...
            device_summary="Intel NPU/GPU/CPU",
            incompatible=False,
            sources=(
                SourceRow(
                    status="primary",
        ...
        EPBlock(
            ...
            device_summary="Qualcomm NPU/GPU/CPU",
            incompatible=True,
            sources=(
                SourceRow(
                    status="incompatible",
...
_EP_NAME_W = 56      # left column for "EP name [incompatible]"
...
def _render_ep_block(block: EPBlock) -> None:
    ...
    left = _pad_visible(f"  {name}", _EP_NAME_W)
    summary = f"[cyan]{block.device_summary}[/cyan]"
    console.print(f"{left} -> {summary}")
