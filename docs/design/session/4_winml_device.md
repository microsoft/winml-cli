...
- A factory `wrap_ort_device(handle)` that constructs a `WinMLDevice` from an `ort.OrtEpDevice`.
...
class WinMLDevice:
    ...
    OS-bound: cannot be hand-constructed; instantiated only via
    :func:`wrap_ort_device(handle)` after a successful ORT registration
    produced the underlying OrtEpDevice. Tests may instantiate directly
    when the OrtEpDevice handle is mocked.
    """
...
def wrap_ort_device(d: ort.OrtEpDevice) -> WinMLDevice:
    """Wrap an ort.OrtEpDevice in the single WinMLDevice concrete class.

    The wrapper is unconditional — there is no per-EP dispatch at
    construction time; the vendor-specific properties dispatch on
    self.ep_name at access time via the §5 tables.
    """
    return WinMLDevice(d)
```

The factory is intentionally trivial under the single-class design. It exists for symmetry with the rest of the session module (everything system-generated has a named factory) and so consumers do not call `WinMLDevice(handle)` directly — `wrap_ort_device(handle)` is the canonical construction path. In the prior ABC + subclass design, the factory carried a `_DEVICE_CLASSES` dispatch map; that map collapsed into the per-property dispatch tables in §5.
...
`wrap_ort_device(ort_dev).available_metadata()` returns the raw `ep_metadata` dict for power users who want to see every field the plugin exposes. Implements requirement #4 from the user's list ("a unified metadata list method indicating what metadata the device provides").
...
src/winml/modelkit/session/
    ...
    winml_device.py      (NEW — this doc's single WinMLDevice class +
                          module-level dispatch tables + wrap_ort_device
                          factory)
src/winml/modelkit/
...
Re-exported from `session/__init__.py` so consumers do `from winml.modelkit.session import WinMLDevice, wrap_ort_device`. There are no per-EP subclass exports — the prior `OpenVINODevice` / `QNNDevice` / `DmlDevice` / `CpuDevice` / `AzureDevice` / `UnknownDevice` names are deleted, not deprecated.
