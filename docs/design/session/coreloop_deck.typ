...
#slide[
  ...
  #table(
    ...
    [`wrap_ort_device(handle)`], [`WinMLDevice` — adapter factory],
...
#slide[
  ...
  *Pending follow-ups* (backlogged tasks):
  ...
  - *Batch G* — fix the `--list-ep` duplicate-MSIX-DLL render; wire
    `IncompatibleListingPick` / `AmbiguousListingPick` raises
