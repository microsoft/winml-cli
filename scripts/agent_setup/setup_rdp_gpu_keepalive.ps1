...
#
# Purpose:
#   Recover the interactive agent session after an RDP disconnect. Session
#   transitions can leave stale Remote Display Adapter nodes and duplicate GPU
#   routes. Returning to console and cleaning non-present RDP nodes helps restore
#   the environment, but does not guarantee that every ORT/DXGI route is usable.
#
# Enhanced worker:
#   Handles network-backed Event 24 immediately, ignores local transitions,
#   protects another user's active console, verifies/retries tscon, and removes
#   only non-present RDP display nodes after a verified console transition.
#   Keep this worker aligned with the deployed C:/agent/tools/keep_console.ps1.
#   The separate CI physical-GPU pin is still needed for stale ORT routes.
#
...
'@
    # Single-quoted here-string keeps event value substitutions literal for Task Scheduler;
    ...
    # ---- safe smoke test: no SessionId means no redirect or display cleanup ----
