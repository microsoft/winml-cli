...
The provided script registers a SYSTEM, event-triggered Scheduled Task (`KeepSessionOnConsole`) that fires on RDP disconnect (TerminalServices-LocalSessionManager **Event ID 24**). It reads the event's source-network address to distinguish a genuine remote disconnect from the `LOCAL` events generated by `tscon` and reconnect transitions. A genuine disconnect is redirected immediately to the physical console, before Windows settles on a phantom Remote Display Adapter topology; local transition events are ignored so reconnecting does not cause a tug-of-war. After verifying the console transition, the worker also removes non-present `SWD\REMOTEDISPLAYENUM` display nodes as best-effort cleanup.
...
The script writes the worker to `C:\agent\tools\keep_console.ps1` and logs activity to `C:\agent\tools\keep_console.log`. To verify, disconnect and reconnect your RDP session, then check the log. A remote IP should be redirected immediately, followed by a verified `console` transition; subsequent events with a local source should be ignored. The worker refuses to displace another user's active console session and never removes the physical PCI display adapter.
