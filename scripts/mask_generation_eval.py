...
def preprocess_image(
    img: Image.Image, profile: MaskGenProfile,
) -> tuple[np.ndarray, float, int, int]:
    """Resize longest side to target, pad to square, normalize.

    Returns:
        pixel_values: (1, 3, T, T) fp32, NCHW
        scale: original_pixel * scale -> resized_pixel
    ...
    arr = (arr - np.array(profile.mean, dtype=np.float32)) / np.array(
        profile.std, dtype=np.float32,
    )

    pad_h = profile.target_size - new_h
    pad_w = profile.target_size - new_w
    arr = np.pad(arr, ((0, pad_h), (0, pad_w), (0, 0)), mode="constant")

    pixel_values = arr.transpose(2, 0, 1)[None, ...]
    return pixel_values.astype(np.float32), scale, new_h, new_w
...
def encode_image(
    ...
    return EncoderState(emb, scale, new_h, new_w, elapsed)
...
def make_prompts(
    ...
    encoder-input space (i.e. ``original * scale``).
    """
    ...
    if prompt_type in ("point", "point+box"):
        ...
        if pt is None:
            return None
        px, py = pt
        points = np.array([[px * scale, py * scale]], dtype=np.float32).reshape(1, 1, 1, 2)
...
def decode_with_prompt(
    ...
    iou_scores, pred_masks, _obj_logits = dec_sess.run(
        ["iou_scores", "pred_masks", "object_score_logits"], dec_inputs,
    )
    elapsed = time.monotonic() - t0
    ...
    binary = postprocess_mask(
        best_low_res, profile, orig_h, orig_w, state.new_h, state.new_w,
    )
...
def _load_coco(n: int, min_area: float = 1024.0, seed: int = 0) -> list[EvalSample]:
    ...
    for ann in chosen:
        ...
        if isinstance(seg, list):
            ...
            rle = seg if isinstance(seg.get("counts"), bytes) else mask_utils.frPyObjects(
                seg, h, w,
            )
        else:
            continue
...
def main() -> int:
    ...
    parser.add_argument(
        ...
        default=None,
        help="Built-in profile. Overrides --encoder/--decoder/--target-size/--mean/--std.",
    )
    ...
    enc_sess = ort.InferenceSession(
        str(enc_path), sess_options=sess_opts,
        providers=providers, provider_options=provider_options,
    )
    dec_sess = ort.InferenceSession(
        str(dec_path), sess_options=sess_opts,
        providers=providers, provider_options=provider_options,
    )
    ...
    for i, sample in enumerate(samples):
        ...
        for pt in prompt_types:
            prompts = make_prompts(pt, sample.gt_mask, state.scale)
            if prompts is None:
                continue
            try:
