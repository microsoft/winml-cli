    ...
    for row in rows:
        ...
        for target_name, target in targets.items():
            ...
            if not isinstance(precision, str) or (
                ...
                raise ValueError(f"Invalid release precision {precision!r} for {key} in {target_name}")
...
def _release_target_column(
    ...
    if machine:
        ...
        path_parts = {part.casefold() for part in output_dir.parts}
        inferred = [match for match in matches if match[0].casefold() in path_parts]
        if len(inferred) == 1:
            matches = inferred
    if not matches:
        raise ValueError(
    ...
    if len(matches) > 1:
        ...
        raise ValueError(
            f"Release target {target_key} is ambiguous; specify --release-machine "
            f"({machines}) or include one machine name in --output-dir"
        )
    return matches[0][1]
