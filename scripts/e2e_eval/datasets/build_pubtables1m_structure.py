...
The Structure split contains cropped table images with sub-table element
annotations (columns, rows, headers, spanning cells).
...
Usage:
    python scripts/e2e_eval/datasets/build_pubtables1m_structure.py --output <dir>
"""
...
def build_dataset(output_dir: Path) -> None:
    """Download PubTables-1M Structure val split, sample, and save as HF dataset."""
    ...
    # Step 1: Download annotations tar (29 MB, cached by hf_hub)
    print("Downloading PubTables-1M structure annotations (val) ...")
    ...
    with tarfile.open(ann_tar_path, "r:gz") as tar:
        for member in tar.getmembers():
            ...
            if f is None:
                continue
            parsed = _parse_voc_xml(f.read(), label2id)
    ...
    # Step 4: Download images tar (2.6 GB, cached by hf_hub)
    print("Downloading PubTables-1M structure images (val) — ~2.6 GB, one-time ...")
...
_DEFAULT_CACHE_DIR = Path.home() / ".cache" / "winml" / "eval_datasets" / "build_pubtables1m_structure"


def main() -> None:
    parser = argparse.ArgumentParser(description="Build PubTables-1M structure recognition dataset")
    parser.add_argument("--output", type=Path, default=None, help="Output directory (default: ~/.cache/winml/eval_datasets/build_pubtables1m_structure)")
