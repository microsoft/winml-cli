...
`run_llm_eval.py` builds a provider-free ONNX Runtime GenAI bundle with Mobius
and benchmarks it on CPU. The default sweep uses 256, 512, and 1024 prompt
tokens, generates 128 tokens, and records three timed generations after one
warmup. A separate Mobius environment is required because Mobius and winml-cli
may pin different ONNX Runtime packages.
...
uv run python scripts/e2e_eval/build_llm_report.py `
  --result eval_results\llm-cpu\llm_cpu_benchmark.json `
  --output eval_results\llm-cpu\report.html
```

To reuse an existing bundle produced by a previous run, add
`--reuse-bundle`. To benchmark a separately supplied bundle, pass
`--bundle-dir`; the runner rejects bundles that declare a hardware execution
provider so a mislabeled accelerator result cannot be reported as CPU.

Outputs:

- `bundle/` - provider-free ONNX Runtime GenAI bundle
- `perf_ctx<N>.json` - raw `winml perf` report for each context length
- `llm_cpu_benchmark.json` - normalized result with raw samples and CPU/RSS data
