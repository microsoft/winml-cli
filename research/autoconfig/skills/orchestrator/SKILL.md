...
The orchestrator owns the **complete, zero-experience search grid**: the
one-factor-at-a-time cross product `opset (17–21) × {baseline, each single graph
pass}` (~70 combinations), generated by `build_search_space()` in `autoconfig.py`
from `OPSET_RANGE` / `OPTIM_PASSES` (the single source of truth for the universe).
It lists *all* combinations up front; the Explorer then prunes/reorders them by
experience. The per-`(ep, device)` matrices in
`ep_device_knowledge/<ep>_<device>.json` (`hypotheses`) that `catalog_sweep.py`
runs are the **experience-pruned and reordered subsets** of this same grid.
