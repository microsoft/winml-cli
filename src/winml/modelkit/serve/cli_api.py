...
Wraps every wmk CLI command as a REST endpoint using Click's CliRunner.
...
Usage:
    wmk serve                   # launches this app on port 8000
...
from ..cli import main as wmk_cli
...
class CliRequest(BaseModel):
    ...
    Examples::
        ...
        → wmk analyze --model model.onnx --ep qnn --verbose

        {"model": "m.onnx", "no_quant": True, "output_dir": "out"}
        → wmk build --model m.onnx --no-quant --output-dir out
    """
...
app = FastAPI(
    title="ModelKit API",
    description=(
        "Phase 0 — CLI Wrapper. Each endpoint invokes a `wmk` CLI command and "
...
def _invoke(command: str, args: dict[str, Any]) -> CliResponse:
    """Invoke a wmk CLI command via CliRunner and return a CliResponse.
    ...
    result = runner.invoke(wmk_cli, cli_args, catch_exceptions=True)
...
@app.post(
    ...
    tags=["CLI"],
    summary="Invoke a wmk CLI command",
    responses={
...
async def run_cli_command(command: str, body: CliRequest) -> CliResponse:
    """Invoke any `wmk` CLI command over HTTP.
    ...
    is equivalent to `wmk analyze --model ./model.onnx --ep qnn --verbose`.
...
# ---------------------------------------------------------------------------
# Startup banner (called from wmk serve)
# ---------------------------------------------------------------------------
