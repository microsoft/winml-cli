...
def build(
    ...
    try:
        ...
        autogenerated_components: dict[str, str] | None = None
        ...
        if config_file is not None:
            ...
            if model_input is not None and model_input.kind is ModelInputKind.ONNX_FILE:
                ...
                try:
                    autogenerated_components = resolve_composite_components(
                        model,
                        task=None,
                        trust_remote_code=trust_remote_code,
                    )
                ...
                outer_task = (
                    next(iter(autogenerated_components.values()))
                    if autogenerated_components
                    else None
                )
                config_or_configs = generate_build_config(
                    model,
        ...
        if isinstance(config_or_configs, list):
            ...
            components = autogenerated_components
            if model and not model_is_onnx and config_file is not None:
                try:
                    from ..loader.resolution import resolve_composite_components
            ...
            if components:
                ...
                composite_model_type = getattr(preloaded_hf_config, "model_type", None)
                if composite_model_type is None and config.loader is not None:
                    composite_model_type = config.loader.model_type
                precision_overrides = (
                    resolve_composite_precision_overrides(composite_model_type, components)
                    if composite_model_type
                    else {}
                )
                try:
                    for name, component_task in components.items():
                        ...
                        component_precision = precision_overrides.get(name, precision)
