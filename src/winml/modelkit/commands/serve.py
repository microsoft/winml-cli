...
Usage:
    ...
    winml serve microsoft/resnet-50         # Phase 1: HF model
    winml serve ./build/resnet50/           # Phase 1: build output dir
    winml serve model.onnx --task image-classification  # Phase 1: raw ONNX
    winml serve microsoft/resnet-50 --multi # Phase 3: multi-model manager
"""
...
def serve(
    ...
    \b
        ...
        winml serve microsoft/resnet-50

        # Phase 1: from build output directory
        winml serve ./build/resnet50/

        # Phase 1: raw ONNX file (task required)
        winml serve model.onnx --task image-classification

        # Phase 2: with idle timeout
        winml serve microsoft/resnet-50 --idle-timeout 300

        # Phase 3: multi-model
        winml serve microsoft/resnet-50 --multi

        # Custom host/port
        winml serve microsoft/resnet-50 --host 0.0.0.0 --port 9000
    """
    try:
        import uvicorn
    except ImportError:
