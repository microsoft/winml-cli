...
def available_eps_for_device(device: str) -> list[str]:
    """Registered EPs for the given device, in catalog priority order.

    Layers :meth:`WinMLEPRegistry.available_eps` (the ORT-registration filter)
    on top of :func:`eps_for_device` (pure catalog). Callers get the same
    availability guarantee the old ``sysinfo.resolve_eps`` provided, without
    depending on the sysinfo shim.
    ...
    Returns:
        List of canonical EP names that both (a) target *device* per the
        static catalog, and (b) are currently registered with ORT. Empty
        list for unknown devices or devices with no registered EPs.
    """
    from .ep_registry import WinMLEPRegistry
...
class WinMLDevice:
    ...
    def ep_name(self) -> str:
        ...
        return cast("str", self._ort.ep_name)

    @property
    def device_type(self) -> str:
        """'NPU' | 'GPU' | 'CPU' - uppercased from device.type.name."""
        return cast("str", self._ort.device.type.name.upper())
    ...
    def architecture(self) -> str | None:
        ...
        if "OpenVINO" in ep:
            ...
            if "arch=" in raw:
                return cast("str", raw.split("arch=", 1)[1].strip())
            return cast("str", raw)
        return None
    ...
    def driver_version(self) -> str | None:
        ...
        if "OpenVINO" in ep and self.device_type == "NPU":
            return cast("str | None", self._ort.ep_metadata.get("NPU_DRIVER_VERSION"))
        return None
    ...
    def compiler_version(self) -> str | None:
        ...
        ep = self._ort.ep_name
        if "OpenVINO" in ep and self.device_type == "NPU":
            return cast("str | None", self._ort.ep_metadata.get("NPU_COMPILER_VERSION"))
        return None
...
from .monitor.report import _format_bytes as _format_bytes  # noqa: E402  (explicit re-export)
