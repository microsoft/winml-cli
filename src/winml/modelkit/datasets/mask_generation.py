...
class MaskGenerationDataset(BaseTaskDataset):
    ...
    def __init__(
        ...
        **kwargs: Any,
        ...
        Args:
            ...
            image_col: Explicit image column override.
            mask_col: Explicit ground-truth mask column override.
        ...
        if prompt_mode not in VALID_PROMPT_MODES:
            raise ValueError(f"prompt_mode={prompt_mode!r} is not one of {VALID_PROMPT_MODES}")
        ...
        if kwargs.get("streaming", False):
            raise ValueError(
                "MaskGenerationDataset does not support streaming=True yet; "
                "mask-generation evaluation needs random access for sample "
                "capping and coverage filtering."
            )
        ...
        self._image_col_override = image_col
        self._mask_col_override = mask_col
    ...
    def _initialize(self) -> None:
        ...
        dataset = load_dataset(
            self._dataset_name,
            name=self._config.get("dataset_config_name"),
            split=self._data_split,
            revision=self._config.get("revision"),
            streaming=bool(self._config.get("streaming", False)),
        )
    ...
    def _detect_columns(self, dataset: Any) -> None:
        ...
        missing = [
            col
            for col in (self._image_col_override, self._mask_col_override)
            if col is not None and col not in features
        ]
        if missing:
            raise ValueError(
                f"Configured mask-generation column(s) not found: {missing}. "
        ...
        if self._image_col_override is not None:
            self._image_col = self._image_col_override
        if self._mask_col_override is not None:
            self._mask_col = self._mask_col_override
        if self._image_col == self._mask_col:
            raise ValueError(
                "mask-generation image_col and mask_col must refer to different columns; "
                f"both resolved to {self._image_col!r}."
            )
