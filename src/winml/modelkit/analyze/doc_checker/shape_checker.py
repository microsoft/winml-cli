...
class ShapeConstraintChecker:
    ...
    def check_rank_range(
        ...
        if min_rank > max_rank:
            raise ValueError(
                f"min_rank ({min_rank}) cannot be greater than max_rank ({max_rank})"
            )
    ...
    def check_dimension_multiple(
        ...
        Returns:
            Tuple of (success, message)
        """
        return ShapeConstraintChecker.check_dimension_divisible(
            shape, dim_index, multiple_of
        )
