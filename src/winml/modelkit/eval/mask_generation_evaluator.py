...
class WinMLMaskGenerationEvaluator(WinMLEvaluator):
    ...
    def __init__(
        ...
        logger.info(
            ...
            self._profile.name, self._profile.target_size,
            self._profile.mean, self._profile.std, self._profile.resize_mode,
        )
    ...
    def _load_sessions(self) -> tuple[Any, Any]:
        ...
        providers, provider_options = _build_providers(self.config.ep or "cpu")
        logger.info(
            "Creating ORT sessions for mask-generation (providers=%s)", providers,
        )
    ...
    def _predict(self, sample: dict[str, Any]) -> np.ndarray:
        ...
        enc_out = self._enc_sess.run(None, {"pixel_values": pixel_values})
        enc_names = [o.name for o in self._enc_sess.get_outputs()]
        ...
        iou_scores, pred_masks, _ = self._dec_sess.run(
...
def _build_providers(ep: str) -> tuple[list[str], list[dict[str, Any]]]:
    """Map ``--ep`` to ORT provider list + per-provider options.

    Mirrors :file:`scripts/sam3_smoke_eval.py`.  Falls back to CPU with a
    warning if the requested EP is not present in this ORT install.
    """
    import onnxruntime as ort
    ...
    providers = providers_map.get(ep.lower(), ["CPUExecutionProvider"])
    avail = set(ort.get_available_providers())
    if providers[0] not in avail:
        logger.warning(
            "Requested EP %r not available (have %s); falling back to CPU.",
            providers[0],
            sorted(avail),
        )
