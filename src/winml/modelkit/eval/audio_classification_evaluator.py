...
class WinMLAudioClassificationEvaluator(WinMLEvaluator):
    ...
    def _to_mono(waveform: np.ndarray) -> np.ndarray:
        ...
        channel_axes = [axis for axis, size in enumerate(waveform.shape) if size <= 8]
        if channel_axes:
            channel_axis = min(channel_axes, key=lambda axis: (waveform.shape[axis], axis))
            return np.asarray(waveform.mean(axis=channel_axis), dtype=np.float32)
